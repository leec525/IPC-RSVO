...
output_part_folder = os.path.abspath("rgb_rs/sliced_t1_slow_cam1")  # 保存渲染所得的各相机的行图像文件夹
output_folder = os.path.abspath("rgb_rs/t1_slow_cam1")  # 保存拼接所得的卷帘图像文件夹
output_folder_gs = "rgb_gs/t1_slow/t1_cam1"  # 保存全局图像的文件夹
...
specified_files = [f't1_slow_cam1_frame_{i:06d}.txt' for i in range(38, 634)]
# 获取所有的frame位姿文件
...
# # 获取所有的位姿文件
# frame_files = sorted([f for f in os.listdir(motion_folder) if f.startswith('t1_slow_cam1_frame_')])
...
for frame_file in frame_files:
    ...
    part_filename_gs = f't1_slow_{frame_num:06d}_part_0000.png'
    ...
    with open(frame_path, 'r') as file:
        ...
        for i in range(image_height):
            part_filename = f't1_slow_{frame_num:06d}_part_{i:04d}.png'
        ...
        for i, line in enumerate(lines):
            ...
            output_file = os.path.join(output_part_folder, f"t1_slow_{frame_num:06d}_part_{i:04d}.png")
            render_image(transx, transy, transz, rotx, roty, rotz, output_file, i+1, i+1)

        # 提取全局图像
        if os.path.exists(part_path_gs):
            # 构建新的文件名和路径
            new_filename = f't1_slow_frame_{frame_num:04d}.png'
    ...
    for row in range(image_height):
        part_filename = f't1_slow_{frame_num:06d}_part_{row:04d}.png'
    ...
    # 保存拼接后的图像
    combined_image.save(os.path.join(output_folder, f't1_slow_{frame_num:06d}.png'))
    print(f'Frame {frame_num} has been combined and saved.')

    # 删除行图像
    for i in range(image_height):
        part_filename = f't1_slow_{frame_num:06d}_part_{i:04d}.png'  # 确保使用正确的行号变量
        part_path = os.path.join(output_part_folder, part_filename)
        if os.path.exists(part_path):
            os.remove(part_path)
        else:
